...
- **Ended** - A closed event is in "readonly" mode, intended for archival purposes only. It is not possible to create new meetings, write comments *(only "Private Notes" can be saved)* and chat.
...
- ![Clock icon](../../_data/i-clock.svg) - Time slot generator
    ...
    ✨ It is not possible to generate more than 100 time slots per generator run.
- ![Chair Icon](../../_data/i-chair.svg) - Location Generator
    ...
    ✨ You cannot generate more than 100 locations per generator run.
...
- 🚧 *External registration URL - Not in use at the moment*
...
- **Delete** - Physically deletes the entire event including all links, locations, timeslots, etc.
...
- ✨ **Auto checkin** - When the participant physically arrives at this subevent, the subevent check-in will be found for them. If they have not yet confirmed it, it will be automatically confirmed for them.

- ✨ **Public confirmation** - Public subevent - entry is allowed even if the user is not registered. This is handy for subevents where anyone can arrive. This setting is not suitable for subevents where there is a limited number of seats and only registered users can arrive.

- 🚧 *Public registration - Not used*

- ✨ **Show in Calendar** - Display of the Subevent in the planning calendar of all users. The subevent is displayed only if it is intended for everyone *(Public confirmation must be turned on)* or if the user is on the list of subevent participants.
...
- ✨ **Download list of participants** - Allows you to download a list of selected participants in a CSV file, which can then be used to import users into another event using the import form
...
✨💎 **Import Registrations** - To import user registrations, use the red button in the top right.
...
### 💡 BULK ACTIONS:
- **Delete selected** - deletion of selected records, this action can only be performed if there are no links to these records, otherwise a page with conflicts will be displayed.
...
- ✨ **Slot Change Proposal** - The newly proposed timeslot to which the meeting is planned to be moved, if the value is empty the meeting has no user change proposal *(there can only be one)*
- ✨ **Slot Change Proposal By User** - The user who requested the timeslot change proposal
